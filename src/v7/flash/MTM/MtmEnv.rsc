...
:if ($MtmFacts != nil) do={
	...
	:if ([:typeof $mVal] = "nil") do={
		:set mVal [($MtmFacts->"setEnv") "mtm.root.path" "flash/MTM"]; ##root path to the MTM files, every file is relative to this path (no leading or trailing / please)
		...
		:set mVal [($MtmFacts->"setEnv") "mtm.remote.url" "merlinthemagic/MTM-RouterOS-Scripting/main/src/v7/flash"]; ##path on server to files (no leading or trailing / please)
		...
		:set mVal [($MtmFacts->"setEnv") "mtm.remote.save.enabled" "true"]; ##save files to local path so fetching is not needed next time
		:set mVal [($MtmFacts->"setEnv") "mtm.remote.save.path" ""]; ##path on local device to save files. Differs from the root allowing you to save in RAM if wanted files are then only fetched on reboot (no leading or trailing / please)
