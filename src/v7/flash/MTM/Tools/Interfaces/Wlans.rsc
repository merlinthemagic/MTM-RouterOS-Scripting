...
:set ($s->"getScan") do={
	...
	##do background so we dont loose DFS status. If not using background we get lots of errors: scan not running interrupted
	## cannot catch those errors, then terminate the script, we would need to send them to a child process
	##with background you loose snr and noise floor
	:local mStatus ([/interface/wireless/monitor 0 once as-value]->"status");
	...
	:local datas ([/interface/wireless/scan [find where name=$0] background=yes rounds=1 passive=yes as-value]);
	:local rObjs [:toarray ""];
	:local rObj [:toarray ""];
	:local r 0;
