...
:set ($s->"getByName") do={
	:global MtmFacts;
	:local method "Facts->IP->Pools->getByName";
	:if ($0 = nil) do={
		[($MtmFacts->"throwException") method=$method msg="Name is mandatory"];
	}
	:local id [/ip pool find where name=$0];
	:if ([:len $id] > 0) do={
		:global |MTMS|;
		:local self ($|MTMS|->"|MTMC|");
