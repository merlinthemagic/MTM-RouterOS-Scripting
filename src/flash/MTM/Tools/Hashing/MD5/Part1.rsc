:set ($s->"hash") do={
	...
	:local arrMd5State [:toarray ""];
	:local arrWordArray [:toarray ""];
	:local ch "";
	:local iByteCount 0;
	:local iCharVal 3;
	:local iDec 0;
	...
	:set lBytePosition 0;
	:set mLen [:len $input];
	:set wCount ( ( ( ( $mLen + 8 ) / 64 ) + 1 ) * 16 );
	:set strWordArray "";
	:set arrWordArray [:toarray ""];

	:for i from=1 to=($wCount) do={
	...
	:while ($iByteCount < $mLen) do={
		:set lWordCount ($iByteCount / 4);
		:set lBytePosition (($iByteCount % 4) * 8);
		:if (($lBytePosition < 0) or ($lBytePosition > 31)) do={
			[($MtmFacts->"throwException") method=$method msg=("Calculating byte position # ".$lBytePosition.", must be 0 thru 31")];
		}
		:set ch [:pick $input $iByteCount];
		:if ([:len [:find $arrCharSet $ch]] > 0) do={
			:set iCharVal ([:tonum [:find $arrCharSet $ch]]);
		} else={
			[($MtmFacts->"throwException") method=$method msg=("Input contains undefined ASCII value")];
		}
		:set lShiftedVal ((($iCharVal) << ($lBytePosition)) | (($iCharVal) >> (32-($lBytePosition))));
		:if ($iByteCount = 0) do={
			:set lShiftedVal (([:tonum $lShiftedVal ] + 0) & 0xFFFFFFFF);
			:set arrWordArray (([:tonum $lShiftedVal ]), [:pick $arrWordArray 1 [:len $arrWordArray]]);
		} else={
			:set lShiftedVal (([:tonum [:pick $arrWordArray $lWordCount]] + 0) | ([:tonum $lShiftedVal ] + 0));
			:set lShiftedVal (([:tonum $lShiftedVal ] + 0) & 0xFFFFFFFF);
			:set arrWordArray ([:pick $arrWordArray 0 $lWordCount ], $lShiftedVal, [:pick $arrWordArray ([:tonum $lWordCount ] + 1) [:len $arrWordArray]]);	
		}
	...
	:set lShiftedVal (([:tonum [:pick $arrWordArray $lWordCount]] + 0 ) | ((0x80 << $lBytePosition) | (0x80 >> (32 - $lBytePosition)))); 
	
	:set arrWordArray (([:pick $arrWordArray 0 $lWordCount ]), [:tonum $lShiftedVal ], ([:pick $arrWordArray ([:tonum $lWordCount] + 1) [:len $arrWordArray]]));
	:set arrWordArray [:toarray (([:pick $arrWordArray 0 ($wCount - 2)]), ((([:tonum $mLen] + 0) << 3) | (([:tonum $mLen] + 0) >> 29)),	(([:tonum $mLen] + 0) >> 29))];
	...
	:for i from=0 to=3 do={
		...
		:for j from=0 to=3 do={
			...
			:for k from=0 to=( 4 * ( $iHexDigit - 1 ) ) step=4 do={
				:set sHex ([:pick [:tostr $strHexValues] (($iDec >> $k) & 0xF) ((($iDec >> $k) & 0xF) + 1)].$sHex);
			}
			:set sHex [:tostr $sHex];
